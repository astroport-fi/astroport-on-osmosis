...
use cosmwasm_std::{
    coin, coins, to_json_binary, Addr, Api, Coin, Decimal, Empty, GovMsg, IbcMsg, IbcQuery,
...
impl Helper {
    pub fn new(
            ...
            .map(|coin| {
                let asset_info = match &coin {
                    ...
                    TestCoin::Cw20(..) | TestCoin::Cw20Precise(..) => {
                        unimplemented!("CW20 is not supported")
                    }
                };
                (coin, asset_info)
            })
        ...
        let maker = Addr::unchecked("fake_maker");
    ...
    pub fn coin_balance(&self, coin: &TestCoin, user: &Addr) -> u128 {
        match &self.assets[coin] {
            ...
            AssetInfo::Token { .. } => unimplemented!("CW20 is not supported"),
        }
    }
